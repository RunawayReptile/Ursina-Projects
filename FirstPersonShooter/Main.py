...
Enemy1 = Button(parent=scene,model="Assets/enemy.fbx", texture="white_cube", position=(x,1,z), scale = 0.01, collider="mesh",color=color.gray,highlight_color=color.gray)
#MAP
for i in range(150):
    x = random.randint(-25,25)
    z = random.randint(-25,25)
...
Shotgun_cursor = Entity(model="cube", parent = camera.ui, scale = .1,texture="white_cube", position=(0.6,-0.4,0))
Pistol_cursor = Entity(model="cube", parent = camera.ui, scale = .1,texture="white_cube", position=(0.72,-0.4,0), color=color.cyan)
#GUNS
